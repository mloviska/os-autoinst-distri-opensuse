...
# SLOW_TYPING_SPEED == max_interval => 13
sub type_string_extended {
    my ($string) = @_;
    type_string($string, max_interval => 13, wait_still_screen => 0.05, timeout => 5, similarity_level => 38);
}

sub run {
    my $self         = shift;
    my $module       = "host";
    my $dm           = lc get_required_var('DESKTOP');
    my $hosts_params = {
        ip    => '195.135.221.134',
    ...
    send_key 'alt-i';
    type_string_extended($hosts_params->{ip});
    send_key 'tab';
    type_string_extended($hosts_params->{fqdn});
    send_key 'tab';
    type_string_extended($hosts_params->{alias});
